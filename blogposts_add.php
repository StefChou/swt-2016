...
if(!isset($_FILES['image']) || $_FILES['image']['size'] > $image_size || $_FILES['image']['type'] != $image_type){err++;}

if($err == 0){

	$image_prefix = rand(10000000000, 9999999999);
	$image_path = 'uploads/';
	...
	if($e){
		...
		$sql = 'insert into blogpost (title, content, publish_date, id_category, image) values (:title, :content, :publish_date, :id_category, :image)';
